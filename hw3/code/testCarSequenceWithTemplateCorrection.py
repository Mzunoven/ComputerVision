...
carrec = np.load('carseqrects.npy')
carrec_wcrt = np.load('carseqrects-wcrt.npy')
r_num = [1, 100, 200, 300, 400]
for idx in range(len(r_num)):
    ...
    ax.add_patch(patch1)
    ax.add_patch(patch2)
    fig.savefig('../results/carseq_wrct'+str(i)+'.png', bbox_inches='tight')
